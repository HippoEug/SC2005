...
int VpnToPhyPage(int vpn)
{
  ...
  //you can refer to PageOutPageIn(int vpn) to see how an entry was created in ipt
    ...
    // loop through to find the correct physical page by comparing pid & vpn
...
void InsertToTLB(int vpn, int phyPage)
{
  ...
  //your code to find an empty in TLB or to replace the oldest entry if TLB is full
    // value is the TLB that will be replaced next
    static int FIFOPointer = 0;
    i = FIFOPointer;

    // using loop to find empty entry in TLB
    for (int j = 0; j < TLBSize; j++) {
        // checks if there is an old entry that is invalid, where valid bit is 0
        if (!machine->tlb[j].valid) { // invalid entry found
            i = j;
            break;
        }
        // else FIFOPointer points to the oldest pointer that will be removed next
    }
