...
Thread::Finish ()
{
	...
	/* Add code here to reset the timer interrupt so that the next 
        ...
        */
	interrupt->InterruptRemove();	// remove the pending interrupts 
    // (assume all the pending interrupts are from the timer interrupts)
    timer->ScheduleTimerInterrupt();	// schedule a new interrupt at the next 40 time ticks
	...
	// This condition never observed to be triggered in Experiment 2, so no code is added here
    interrupt->InterruptRemove();    // remove the pending interrupts
    // (assume all the pending interrupts are from the timer interrupts)
    timer->ScheduleTimerInterrupt();    // schedule a new interrupt at the next 40 time ticks
